...
export default function CustomerDashboard() {
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                ...
                // Assuming an API endpoint exists to actually save this
                // api.put("/auth/me/avatar", { avatar: reader.result });
            };
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] py-8 sm:py-12 px-4 sm:px-6 lg:px-8 font-sans">
            <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100">
                <div>
            ...
            <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
                ...
                <div className="w-full lg:w-1/3 flex flex-col gap-6">
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-8 flex flex-col items-center text-center relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-r from-primary-50 to-primary-100/50"></div>
                        <div className="relative z-10 w-full flex flex-col items-center">
                            <div className="relative mb-4 group">
                                {profilePic ? (
                                    <img src={profilePic} alt="Profile" className="h-28 w-28 rounded-full object-cover shadow-md border-4 border-white mx-auto z-10 relative" />
                                ) : (
                                    <div className="h-28 w-28 bg-primary-600 text-white rounded-full flex items-center justify-center text-4xl font-bold uppercase shadow-md border-4 border-white mx-auto z-10 relative">
                                        {firstName.charAt(0)}{lastName.charAt(0)}
                                    </div>
                                )}
                                <label className="absolute bottom-0 right-0 bg-white p-2.5 text-slate-600 hover:text-primary-600 rounded-full shadow-lg border border-slate-100 transition-colors cursor-pointer z-20 hover:scale-110 active:scale-95 translate-x-1 translate-y-1">
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 flex flex-col gap-4">
                        <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                            <Package className="w-5 h-5 text-primary-500" /> Account Overview
                        </h3>

                        <div className="flex bg-slate-50 rounded-2xl p-4 border border-slate-100 justify-between items-center whitespace-nowrap overflow-hidden">
                            ...
                            <div className="text-left">
                                ...
                                <p className="text-sm font-bold text-slate-700 truncate">{orders.length > 0 ? orders[0].status : "No orders yet"}</p>
                            </div>
                        </div>

                        <div className="bg-blue-50/50 rounded-2xl p-4 border border-blue-100/50 flex gap-3 text-left">
                            <MapPin className="w-5 h-5 text-blue-500 shrink-0 mt-0.5" />
                            <div className="overflow-hidden">
                                <p className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-1">Default Delivery Address</p>
                                <p className="text-sm font-bold text-slate-800 truncate">
                                    {defaultAddress ? `${defaultAddress.street}, ${defaultAddress.city}` : "No default address set"}
                                </p>
                            </div>
                        </div>
                    </div>
